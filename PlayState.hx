	...
	override public function create()
	{
		...
		switch (curStage)
		{
			case 'stage': //Week 1
				...
				case 'parkbetter': //Week 1

				var other:BGSprite = new BGSprite('wum', 0, 50, 0.8, 0.8);
				add(other);

				var bg:BGSprite = new BGSprite('back', 0, 50, 0.8, 0.8);
				add(bg);

				var top:BGSprite = new BGSprite('top', 0, 50, 0.8, 0.8);
				add(gfGroup);
				add(dadGroup);
				add(boyfriendGroup);
				add(top);

				case 'parkbetterreload': //Week 1
				...
				case 'space': //Week 1
				var bg:BGSprite = new BGSprite('space', -1659, -708, 1, 1);
				defaultCamZoom = 0.5;
				add(bg);

				var messages:BGSprite = new BGSprite('messages', -1659, -708, 0.85, 0.85, ['messages loop'], true);
			//	messages.setGraphicSize(widShit);
		     	messages.animation.curAnim.frameRate = 6;
				messages.updateHitbox();
				messages.antialiasing = false;

				var over:BGSprite = new BGSprite('spacebf', -1659, -708, 1, 1);
				add(gfGroup);
				add(dadGroup);
				add(boyfriendGroup);
				add(over);
			...
			case 'spooky': //Week 2
				if(!ClientPrefs.lowQuality) {
					halloweenBG = new BGSprite('halloween_bg', -200, -130, ['halloweem bg0', 'halloweem bg lightning strike']);
				} else {
					halloweenBG = new BGSprite('halloween_bg_low', -200, -130);
				}
				add(halloweenBG);
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					if (SONG.song.toLowerCase() == 'boost')
						{
			...
			if(note.hitCausesMiss) {
				...
				switch(note.noteType) {
					case 'Hurt Note': //Hurt note
						...
						case 'Newer Hurt Note': //Hurt note
						if(boyfriend.animation.getByName('hurt') != null) {
							boyfriend.playAnim('hurt', true);
							boyfriend.specialAnim = true;
						}
						case 'Kais Boost Note': //Hurt note
						if(boyfriend.animation.getByName('hurt') != null) {
							boyfriend.playAnim('hurt', true);
							boyfriend.specialAnim = true;
						}
		...
		for (i in 0...achievementsToCheck.length) {
			...
			if(!Achievements.isAchievementUnlocked(achievementName) && !cpuControlled && !unlock) {
				switch(achievementName)
				{
					...
					case 'wum1':
						if(Paths.formatToSongPath(SONG.song) == 'wumpus' && !usedPractice) {
							unlock = true;
						}
					case 'nitro1':
						if(Paths.formatToSongPath(SONG.song) == 'nitro' && !usedPractice) {
							unlock = true;
						}
					case 'boost1':
						if(Paths.formatToSongPath(SONG.song) == 'boost' && !usedPractice) {
							unlock = true;
						}
					case 'wum2':
						if(Paths.formatToSongPath(SONG.song) == 'wumpus' && !usedPractice && CoolUtil.difficultyString() == '@everyone') {
						    unlock = true;
						}
					case 'nitro2':
						if(Paths.formatToSongPath(SONG.song) == 'nitro' && !usedPractice && CoolUtil.difficultyString() == '@everyone') {
	    					unlock = true;
						}
					case 'boost2':
						if(Paths.formatToSongPath(SONG.song) == 'boost' && !usedPractice && CoolUtil.difficultyString() == '@everyone') {
							unlock = true;
						    }
						}
					}
