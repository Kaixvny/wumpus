...
class StoryMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('menubgsm'));
		add(bg);
		...
		for (i in 0...WeekData.weeksList.length)
		{
			...
			var weekThing:MenuItem = new MenuItem(0, bgSprite.y + 27, WeekData.weeksList[i]);
	...
	//	add(grpWeekCharacters);
		...
		txtTracklist = new FlxText(FlxG.width * 0.05, tracksSprite.y + 80, 0, "", 32);
		txtTracklist.x -= 0;
		txtTracklist.alignment = LEFT;
		txtTracklist.font = rankText.font;
		txtTracklist.color = 0x7289da;
		//add(txtTracklist);
		//add(rankText);
		//add(scoreText);
		//add(txtWeekTitle);
